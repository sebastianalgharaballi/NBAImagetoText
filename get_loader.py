...
import torch
from torch.nn.utils.rnn import pad_sequence  # batching with pads
...
class NBADataset(Dataset):
    def __init__(self, root_dir, captions_file, transform=None, freq_threshold=5):
        ...
        self.df = pd.read_csv(captions_file)  # read into the captions file
        ...
        # build vocab after initialization
...
def get_loader(root_folder, annotation_file, transform, batch_size=32, num_workers=8, shuffle=True, pin_memory=True):
    dataset = NBADataset(root_folder, annotation_file, transform=transform)
...
if __name__ == '__main__':
    ...
    loader, dataset = get_loader('NBADataset/images', 'NBADataset/captions.txt', transform = transform)
